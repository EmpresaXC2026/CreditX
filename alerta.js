...
function getSaldo(p, pagos) {
  ...
  return (p.totalConInteres || p.monto || 0) - pagado;
}
...
(async () => {
  try {
    ...
    if (!snap.exists) {
      await send('Sin datos en CreditX');
      return;
    }
    ...
    const cobrosHoy = prestamos.filter(p => p.fechaProximoPago === hoy && p.estado !== 'Pagado');
    const cobrosManana = prestamos.filter(p => p.fechaProximoPago === manana && p.estado !== 'Pagado');
    const atrasados = prestamos.filter(p =>
      p.fechaProximoPago < hoy && p.estado !== 'Pagado' && getSaldo(p, pagos) > 0
    );
    ...
    if (cobrosHoy.length) {
      const total = cobrosHoy.reduce((a, p) => a + (p.cuota || 0), 0);
      msg += 'COBRAR HOY (' + cobrosHoy.length + ')\n';
      cobrosHoy.forEach((p, i) => {
        msg += (i + 1) + '. ' + p.nombre;
        if (p.telefono) msg += ' | ' + p.telefono;
        msg += '\n   Q ' + (p.cuota || 0).toFixed(2) + ' | Saldo: Q ' + getSaldo(p, pagos).toFixed(2) + '\n';
      });
      msg += 'Total hoy: Q ' + total.toFixed(2) + '\n\n';
    }

    if (cobrosManana.length) {
      const total = cobrosManana.reduce((a, p) => a + (p.cuota || 0), 0);
      msg += 'COBRAR MANANA (' + cobrosManana.length + ')\n';
      cobrosManana.forEach((p, i) => {
        msg += (i + 1) + '. ' + p.nombre;
        if (p.telefono) msg += ' | ' + p.telefono;
        msg += '\n   Q ' + (p.cuota || 0).toFixed(2) + ' | Saldo: Q ' + getSaldo(p, pagos).toFixed(2) + '\n';
      });
    ...
    if (atrasados.length) {
      ...
      atrasados.forEach((p, i) => {
        msg += (i + 1) + '. ' + p.nombre + ' | Saldo: Q ' + getSaldo(p, pagos).toFixed(2) + '\n';
      });
    }
